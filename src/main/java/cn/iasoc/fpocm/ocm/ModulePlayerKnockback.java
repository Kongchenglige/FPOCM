 ...
 */
public class ModulePlayerKnockback implements Listener {
    ...
    public ModulePlayerKnockback(FPOCM plugin) {
        this.plugin = plugin;
        reload();
    }
...
//        } else if(!isEnabled(victim)) return;
        ...
        if (netheriteKnockbackResistance) {
            ...
            final double resistance = 1 - Objects.requireNonNull(victim.getAttribute(Attribute.GENERIC_KNOCKBACK_RESISTANCE)).getValue();
        ...
        FPOCM.getScheduler().runTaskLater(plugin, () -> playerKnockbackHashMap.remove(victimId), 1);
    }
}